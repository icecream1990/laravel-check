...
//  登録ボタンを押したとき
 if(isset($_POST['submit'])){
  try{
    ...
    // bindValue()メソッドを使って入力された値をテーブルに割り当てする
    ...
    // 商品一覧ページにリダイレクトさせる（メッセージのパラメータも同時に送る）
    // $messageは$stme_insertとは別の処理のため、2つのパラメータを送る必要がある。
      ...
      <form action ="create.php" method="post" class="registration-form">
        <div>
          ...
          <select id="genre_code" name="genre_code" required>
            <option disabled select value>選択してください</option>
            <?php
